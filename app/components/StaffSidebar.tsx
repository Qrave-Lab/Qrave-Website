...
const sidebarItems: SidebarSection[] = [
  {
    category: "Overview",
    items: [{ label: "Floor & Tables", href: "/staff", icon: LayoutDashboard }],
  },
  {
    category: "Management",
    items: [
      ...
      },
      {
        ...
        icon: CalendarCheck,
        description: "Table holds & waitlist",
      },
      {
      ...
      {
        label: "Cash Drawer",
        href: "/staff/cashier/cash-drawer",
        icon: Banknote,
        description: "Floats, drops and audits",
      },
      {
        label: "Sales & Reports",
        href: "/staff/analytics",
        icon: BarChart3,
        description: "Daily/Monthly, CSV Export",
      },
      {
        label: "Tax Invoices",
        href: "/staff/invoices",
        icon: FileText,
        description: "GST invoices & credit notes",
      },
      {
        label: "Inventory Hub",
        href: "/staff/inventory",
        icon: PackageOpen,
        description: "POs, Suppliers, Recipes",
      },
    ],
  },
  {
...
function hasFeatureAccess(
  role: string,
  roleAccess: RoleAccessConfig | null,
  feature: string,
): boolean {
...
export default function StaffSidebar() {
  ...
  const [isCollapsed, setIsCollapsed] = useState<boolean>(() => {
    ...
    try {
      ...
      return savedState ? Boolean(JSON.parse(savedState)) : true;
    } catch {
      return true;
    }
  });
          ...
          items: section.items.filter((item) => {
            ...
            const feature =
              item.href === "/staff"
                ...
                : item.href === "/staff/menu"
                  ? "menu"
                  : item.href === "/staff/menu/health"
                    ? "insights"
                    : item.href === "/staff/analytics"
                      ? "analytics"
                      : item.href === "/staff/reservations"
                        ? "reservations"
                        : item.href === "/staff/settings"
                          ? "settings"
            ...
            return feature
              ? hasFeatureAccess(currentRole, roleAccess, feature)
              : true;
          }),
        }))
        .filter((section) => section.items.length > 0);
  const activeLocationLabel = (() => {
    const active = locations.find(
      (l) => l.restaurant_id === activeRestaurantId,
    );
  ...
  return (
    ...
    >
      <div
        className={`h-16 flex items-center border-b border-gray-100 ${isCollapsed ? "justify-center" : "px-6"}`}
      >
        ...
        {!isCollapsed && (
          ...
          >
            ...
            <p
              className="mt-0.5 truncate text-[10px] font-semibold text-slate-600"
                    ...
                    {locations.map((loc) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            handleSwitchLocation(loc.restaurant_id)
                          }
                        ...
                        >
                          <MapPin
                            className={`w-3 h-3 shrink-0 ${isActive ? "text-white/60" : "text-slate-400"}`}
                          />
                          <div className="flex-1 min-w-0">
                            <p
                              className={`text-[11px] font-semibold truncate ${isActive ? "text-white" : "text-slate-800"}`}
                            >
                              {loc.restaurant}
                            </p>
                            {lbl && (
                              <p
                                className={`text-[10px] truncate ${isActive ? "text-white/60" : "text-slate-400"}`}
                              >
                                {lbl}
                              </p>
                            )}
      ...
      <div
        className={`flex-1 overflow-y-auto py-6 flex flex-col ${isCollapsed ? "gap-2" : "gap-6"}`}
      >
            ...
            <div className="space-y-1">
              {section.items.map((item) => {
                const isActive =
                  ...
                  (item.href === "/staff/menu" &&
                    pathname.startsWith("/staff/menu/") &&
                    !pathname.startsWith("/staff/menu/health")) ||
                  (item.href === "/staff/settings" &&
                    pathname.startsWith("/staff/settings/") &&
                    pathname !== "/staff/settings/shifts");
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-5 h-5 shrink-0 ${isActive ? "text-white" : "text-gray-500 group-hover:text-gray-900"}`}
                    />
                    ...
                    {!isCollapsed && (
                      <div className="flex-1 overflow-hidden whitespace-nowrap">
                        ...
                        {item.description && (
                          <span
                            className={`text-[10px] block truncate ${isActive ? "text-gray-400" : "text-gray-400"}`}
                          >
                            {item.description}
                          </span>
                        )}
