...
type DeliveryZone = { id: string; name: string; distance_km?: number | null; fee: number; estimated_minutes?: number | null; sort_order: number };
type ZoneForm = { name: string; distance_km: string; fee: string; estimated_minutes: string };
...
export default function DeliveryZonesPage() {
    ...
    useEffect(() => {
        (async () => {
            try { const res = await api<{ zones?: DeliveryZone[] }>("/api/admin/delivery/zones"); setZones(res?.zones || []); }
            catch { toast.error("Failed to load zones"); } finally { setIsLoading(false); }
        })();
    }, []);

    const startEdit = (zone: DeliveryZone) => {
        setEditingId(zone.id);
        setForm({ name: zone.name, distance_km: zone.distance_km != null ? String(zone.distance_km) : "", fee: String(zone.fee), estimated_minutes: zone.estimated_minutes != null ? String(zone.estimated_minutes) : "" });
        setShowAdd(false);
    };
    const startAdd = () => { setEditingId(null); setForm(emptyForm); setShowAdd(true); };
    ...
    const saveZone = async (isNew: boolean) => {
        ...
        setIsSaving(true);
        try {
            const payload: any = { name: form.name.trim(), fee, sort_order: isNew ? zones.length : zones.find(z => z.id === editingId)?.sort_order ?? 0 };
            if (form.distance_km.trim()) { const d = parseFloat(form.distance_km); if (!isNaN(d)) payload.distance_km = d; }
            if (form.estimated_minutes.trim()) { const m = parseInt(form.estimated_minutes, 10); if (!isNaN(m)) payload.estimated_minutes = m; }
            const saved = await api<DeliveryZone>("/api/admin/delivery/zones", { method: "POST", body: JSON.stringify(payload) });
            toast.success(isNew ? "Zone added" : "Zone updated");
            setZones(prev => isNew ? [...prev.filter(z => z.id !== saved.id), saved] : prev.map(z => z.id === saved.id ? saved : z));
            cancelEdit();
        } catch (err: any) { toast.error(err?.message || "Failed to save zone"); } finally { setIsSaving(false); }
    };

    const deleteZone = async (id: string) => {
        try { await api(`/api/admin/delivery/zones/${id}`, { method: "DELETE" }); setZones(prev => prev.filter(z => z.id !== id)); toast.success("Zone removed"); }
        ...
        <div className="bg-indigo-50/40 border border-indigo-100 rounded-2xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input type="number" min="0" step="0.1" value={form.distance_km} onChange={e => setForm(f => ({ ...f, distance_km: e.target.value }))} placeholder="e.g. 3.5" className="w-full px-4 py-2.5 text-sm bg-white border border-slate-200 rounded-xl outline-none" />
                </div>
                <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Delivery Fee (₹) *</label>
                    <input type="number" min="0" step="0.5" value={form.fee} onChange={e => setForm(f => ({ ...f, fee: e.target.value }))} placeholder="e.g. 50" className="w-full px-4 py-2.5 text-sm bg-white border border-slate-200 rounded-xl outline-none" />
                </div>
                <div className="col-span-2">
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5"><Clock className="w-3 h-3 inline mr-1" />Estimated Delivery Time (minutes)</label>
                    <input type="number" min="0" value={form.estimated_minutes} onChange={e => setForm(f => ({ ...f, estimated_minutes: e.target.value }))} placeholder="e.g. 30" className="w-full px-4 py-2.5 text-sm bg-white border border-slate-200 rounded-xl outline-none" />
                </div>
            </div>
    ...
    return (
        ...
        >
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-3 mt-0.5">
                                                    {zone.distance_km != null && <span className="text-[11px] text-slate-500 flex items-center gap-1"><MapPin className="w-3 h-3" />{zone.distance_km} km</span>}
                                                    {zone.estimated_minutes != null && <span className="text-[11px] text-slate-500 flex items-center gap-1"><Clock className="w-3 h-3" />{zone.estimated_minutes} min</span>}
                                                </div>
                                            </div>
                                        </div>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
                ...
                <p className="text-xs text-blue-700 leading-relaxed">Create zones for different delivery areas. Staff select the applicable zone when placing a delivery order — the fee is automatically added to the order total.</p>
            </div>
        </SettingsPageLayout>
    );
}
