...
import { ArrowLeft, Loader2, Plus, TicketPercent, Trash2 } from "lucide-react";
...
export default function OffersSettingsPage() {
      ...
      scope === "selected_items"
        ? selectedItems.map((id) => {
            ...
            if (val > 0) {
              return {
                menu_item_id: id,
                item_discount_kind: "fixed_price",
  ...
  const updateOffer = async (offer: OfferCampaign, patch: Partial<OfferCampaign>) => {
    ...
    await api(`/api/admin/offers/${offer.id}`, { method: "PUT", body: JSON.stringify(payload) });
  };
  ...
  const discountValueLabel =
    discountKind === "percent"
      ...
      : discountKind === "fixed"
        ? "Discount amount (INR)"
        : "Discounted selling price (INR)";

  const offerExplainText = (() => {
    const scopeText = scope === "full_menu" ? "entire menu" : "selected dishes only";
    const applyText = requiresCoupon ? `using coupon code "${couponCode || "YOURCODE"}"` : "automatically (no coupon needed)";
    return `This offer applies to ${scopeText} and is redeemed ${applyText}.`;
  })();

  const prettyKind = (kind: OfferCampaign["discount_kind"]) => {
    if (kind === "percent") return "Percent Off";
    if (kind === "fixed") return "Flat Amount Off";
    ...
    <div className="flex h-screen w-full bg-[#f8fafc] text-slate-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900">Offers & Coupons</h1>
            <p className="text-sm text-slate-500">Create full-menu or dish-level discounts with optional coupon codes.</p>
          </div>
          <Link
            href="/staff/settings"
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="mx-auto max-w-[1400px] space-y-6">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <div className="space-y-4 rounded-2xl border border-slate-200 p-4">
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold text-slate-600">Offer Name</label>
                    <input
                      value={name}
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold text-slate-600">Offer Scope</label>
                      <select value={scope} onChange={(e) => setScope(e.target.value as any)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                        <option value="full_menu">Full Menu</option>
                        <option value="selected_items">Only Selected Dishes</option>
                      </select>
                    </div>
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold text-slate-600">Discount Type</label>
                      <select value={discountKind} onChange={(e) => setDiscountKind(e.target.value as any)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                        <option value="percent">Percent Off (%)</option>
                        <option value="fixed">Flat Amount Off (INR)</option>
                        <option value="fixed_price">Fixed Selling Price (INR)</option>
                      </select>
                    </div>
                  </div>
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold text-slate-600">{discountValueLabel}</label>
                    <input
                      type="number"
                      min={0}
                      step="0.01"
                ...
                <div className="space-y-4 rounded-2xl border border-slate-200 p-4">
                  ...
                  <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm">
                    ...
                    Require customers to enter a coupon code
                  </label>
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold text-slate-600">Coupon Code</label>
                    <input
                      value={couponCode}
                      onChange={(e) => setCouponCode(e.target.value.toUpperCase())}
                      className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm"
                      placeholder="SAVE20"
                      disabled={!requiresCoupon}
                    />
                  </div>
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold text-slate-600">Start Time (optional)</label>
                      <input type="datetime-local" value={startsAt} onChange={(e) => setStartsAt(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
                    </div>
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold text-slate-600">End Time (optional)</label>
                      <input type="datetime-local" value={endsAt} onChange={(e) => setEndsAt(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
                    </div>
                  </div>
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold text-slate-600">Max Coupon Redemptions (optional)</label>
                    <input
                      type="number"
                      min={1}
              ...
              <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-3 text-sm text-indigo-700">
                {offerExplainText}
              </div>
                      ...
                      return (
                        <div key={m.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 px-3 py-2">
                          <label className="inline-flex items-center gap-2 text-sm">
                            ...
                            />
                            <span>{m.name}</span>
                            {m.categoryName ? <span className="text-xs text-slate-400">({m.categoryName})</span> : null}
                          </label>
                          {checked && (
                            <input
                              type="number"
                              ...
                              placeholder="Optional per-item fixed price (INR)"
                            />
                          )}
                        </div>
                      );
              ...
              <button
                ...
                className="mt-5 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white hover:bg-slate-800 disabled:opacity-60"
              >
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {isLoading ? (
                <div className="py-12 text-center text-sm text-slate-500">Loading offers...</div>
              ) : offers.length === 0 ? (
                <div className="py-12 text-center text-sm text-slate-500">No offers configured yet.</div>
              ) : (
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            {offer.scope === "full_menu" ? "Full Menu" : `${offer.items?.length || 0} Selected Dish(es)`}
                            {" • "}{prettyKind(offer.discount_kind)}: {offer.discount_value}
                            {offer.requires_coupon && offer.coupon_code ? ` • Code: ${offer.coupon_code}` : ""}
                          </p>
                          {(offer.starts_at || offer.ends_at) && (
                            <p className="text-xs text-slate-400 mt-1">
                              {offer.starts_at ? toDateTimeLocal(offer.starts_at) : "Now"} to {offer.ends_at ? toDateTimeLocal(offer.ends_at) : "No end"}
                            </p>
                          )}
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className={`rounded-lg px-3 py-1.5 text-xs font-bold ${offer.is_active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}
                          >
