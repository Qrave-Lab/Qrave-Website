...
const templates = [
  { id: "modern", name: "Clean Minimal", class: "bg-white text-gray-900 border border-slate-100" },
  { id: "dark", name: "Midnight Luxury", class: "bg-gray-950 text-white" },
  { id: "framed", name: "Bold Frame", class: "bg-white text-gray-900 border-8" },
  { id: "gold", name: "Imperial Gold", class: "bg-black text-[#F3E5AB] border-4 border-[#D4AF37]" },
  { id: "eco", name: "Organic Green", class: "bg-[#FAF9F6] text-emerald-950 border-4 border-emerald-900" },
];
...
const brandColors = ["#000000", "#10B981", "#6366F1", "#F43F5E", "#F59E0B", "#D4AF37", "#1E3A8A"];
...
export default function QrFlyerGenerator() {
  ...
  const [restaurantLogoUrl, setRestaurantLogoUrl] = useState<string>("Restaurant");
  ...
  if (!selectedTable) {
    return <div className="flex h-screen items-center justify-center text-gray-500 font-semibold bg-slate-50">Loading tables…</div>;
  }
  ...
  const FlyerPanelContent = ({ table, isHalf }: { table: Table; isHalf: boolean }) => {
    ...
    return (
      <div className={`relative flex flex-col items-center h-full w-full justify-between ${isHalf ? "p-4" : "py-10 px-8"}`}>
        {/* Brand Header */}
        <div className={`flex items-center justify-center ${isHalf ? "h-8 mb-2" : "h-14 mb-4"}`}>
          {logoSrc ? (
            <img src={logoSrc} className="h-full object-contain" alt="Logo" />
          ) : (
            <h1 className={`${isHalf ? "text-base" : "text-2xl"} font-black tracking-widest uppercase`} style={{ color: template === "gold" ? "#D4AF37" : "inherit" }}>
              {restaurantName}
            </h1>
          )}
        </div>
        ...
        <div
          ...
          style={{
            boxShadow: template === "modern" ? `0 20px 40px -10px ${brandColor}30` : "",
        ...
        >
          ...
          />
          <div className={`py-1 px-4 rounded-full font-bold uppercase tracking-wider inline-flex items-center gap-1.5 ${isHalf ? "text-[8px] mt-2.5" : "text-[10px] mt-4"}`} style={{ backgroundColor: brandColor, color: "#fff" }}>
        ...
        <div className={`max-w-[90%] space-y-1.5 ${isHalf ? "mb-1" : "mb-4"}`}>
          ...
          <h2 className={`${isHalf ? "text-sm" : "text-xl"} font-black leading-tight`} style={{ color: template === "modern" ? brandColor : "inherit" }}>
            {headline}
          </h2>
          <p className={`leading-relaxed ${isHalf ? "text-[9px]" : "text-xs"} ${isDark ? "text-slate-300" : "text-slate-500"}`}>
            {subheadline}
          </p>
        </div>
        ...
        {/* Wi-Fi Details */}
        {wifiSsid && (
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg border ${isHalf ? "text-[8px] py-0.5" : "text-xs"} ${
            isDark ? "border-slate-800 bg-slate-900/60 text-slate-200" : "border-slate-200 bg-slate-50 text-slate-700"
          }`}>
            <Wifi className={isHalf ? "w-3 h-3 text-slate-400" : "w-3.5 h-3.5 text-slate-400"} />
        ...
        <div className="w-full pt-3 mt-auto border-t border-current border-opacity-10 flex justify-between items-end opacity-80">
          <div className="text-left">
            <div className="text-[8px] uppercase tracking-widest opacity-60">Table</div>
            <div className={`${isHalf ? "text-lg" : "text-3xl"} font-black leading-none mt-0.5`}>
              {getTableLabel(table)}
            </div>
          </div>
          <div className="text-right">
            {table.floor_name && (
              <>
                <div className="text-[8px] uppercase tracking-widest opacity-60">Floor</div>
                <div className={`${isHalf ? "text-[10px]" : "text-xs"} font-black leading-none mt-1`}>
                  {table.floor_name}
                </div>
              </>
            )}
      ...
      >
        {bgImage && (
          <>
            ...
            <div className="absolute inset-0 z-0 transition-colors duration-300" style={{ backgroundColor: template === "dark" ? "#000" : "#fff", opacity: overlayOpacity / 100 }} />
          </>
        )}
        {template === "framed" && <div className="absolute inset-4 border-4 z-10 pointer-events-none" style={{ borderColor: brandColor }} />}
        {template === "gold" && <div className="absolute inset-4 border border-dashed border-[#D4AF37]/40 z-10 pointer-events-none" />}
        {template === "eco" && <div className="absolute inset-4 border border-dashed border-emerald-800/30 z-10 pointer-events-none" />}
    ...
    >
      ...
      <style jsx global>{`
        ...
        @media print {
          ...
          #bulk-print-container * { visibility: visible !important; }
          #bulk-print-container {
      ...
      <div className="no-print flex gap-6 min-h-[720px]">
        ...
        <div className="w-96 shrink-0 bg-white border border-slate-200 rounded-3xl flex flex-col overflow-hidden shadow-sm">
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            ...
            <div className="border-b border-gray-100">
              <button onClick={() => toggleSection("layout")} className="w-full flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3 font-semibold text-sm text-gray-700">
                  <div className="w-6 h-6 rounded bg-indigo-50 text-indigo-600 flex items-center justify-center"><Grid className="w-3.5 h-3.5" /></div>
                  Format & Layout Mode
                </div>
                {activeSection === "layout" ? <ChevronDown className="w-4 h-4 text-gray-400" /> : <ChevronRight className="w-4 h-4 text-gray-400" />}
              </button>
              {activeSection === "layout" && (
                <div className="px-6 pb-6 space-y-4">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Print Mode</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Tent Folding Style</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                  ...
                  <div className="pt-3 border-t border-slate-100 space-y-3">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-xs font-bold text-slate-600">Include "3D AR Enabled" Badge</span>
                    </label>

                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Promo Ribbon Text (Optional)</label>
                      <input
                        value={promoText}
            ...
            <div className="border-b border-gray-100">
              <button onClick={() => toggleSection("target")} className="w-full flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3 font-semibold text-sm text-gray-700">
                  ...
                  {printMode === "single" ? "Target Table" : "Select Tables for Bulk Print"}
                </div>
              ...
              {activeSection === "target" && (
                <div className="px-6 pb-6">
                  {printMode === "single" ? (
                    <>
                      <p className="text-xs text-gray-500 mb-3">Select which table flyer is being designed.</p>
                      {/* Floor tabs */}
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                        <span className="text-xs font-bold text-slate-500">{selectedTableIds.size} / {tables.length} selected</span>
                        <div className="flex gap-2">
                          <button onClick={selectAllTables} className="text-[10px] bg-slate-900 text-white px-2.5 py-1 rounded-lg font-bold">Select All</button>
                          <button onClick={deselectAllTables} className="text-[10px] bg-white text-slate-600 border border-slate-200 px-2.5 py-1 rounded-lg font-bold hover:bg-slate-50">Clear</button>
                        </div>
                      </div>
            ...
            <div className="border-b border-gray-100">
              ...
              {activeSection === "design" && (
                <div className="px-6 pb-6 space-y-5">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Layout Style</label>
                    <div className="grid grid-cols-2 gap-2">
                      {templates.map(t => (
                        <button
                          key={t.id}
                      ...
                      {fonts.map(f => (
                        <button
                          ...
                          className={`flex-1 py-2 text-xs font-bold transition-colors ${f.class} ${
                            activeFont === f.id ? "bg-slate-900 text-white" : "bg-white text-slate-600 hover:bg-gray-50"
                          }`}
                        >
                          Aa
                        </button>
                      ...
                      {brandColors.map(c => (
                        <button
                          ...
                          className={`w-8 h-8 rounded-full border-2 transition-transform hover:scale-105 ${
                            brandColor === c ? "ring-2 ring-offset-2 ring-slate-400" : ""
                          }`}
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Background Image</label>
                    ...
                    {!bgImage ? (
                      <button onClick={() => fileInputRef.current?.click()} className="w-full border-2 border-dashed border-gray-350 rounded-2xl p-5 flex flex-col items-center justify-center text-gray-500 hover:bg-gray-50 hover:border-gray-400 transition-colors gap-2"><ImageIcon className="w-5 h-5 text-gray-400" /><span className="text-xs font-semibold">Click to upload image</span></button>
                    ) : (
                      <div className="space-y-3">
                        <div className="relative h-24 w-full rounded-2xl overflow-hidden border border-gray-250"><img src={bgImage} className="w-full h-full object-cover" alt="bg upload" /><div className="absolute inset-0 bg-black/10" /></div>
                        <div><div className="flex justify-between text-xs mb-1.5"><span className="text-gray-500 font-medium">Overlay Strength</span><span className="font-bold">{overlayOpacity}%</span></div><input type="range" min="0" max="90" value={overlayOpacity} onChange={(e) => setOverlayOpacity(Number(e.target.value))} className="w-full accent-slate-900 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer" /></div>
                      </div>
                    )}
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Brand Logo (Center Logo)</label>
                      {logoImage && <button onClick={() => setLogoImage(null)} className="text-[10px] text-red-500 font-medium hover:underline">Clear</button>}
                    </div>
                    <div className="flex items-center gap-3">
                      {logoImage ? <div className="w-12 h-12 border rounded-xl p-1 bg-white flex items-center justify-center"><img src={logoImage} className="w-full h-full object-contain" alt="logo upload" /></div> : <div className="w-12 h-12 bg-gray-50 rounded-xl flex items-center justify-center text-gray-400 border border-slate-200"><Upload className="w-4 h-4" /></div>}
                      <button onClick={() => logoInputRef.current?.click()} className="text-xs bg-white border border-gray-300 px-3.5 py-2 rounded-xl font-bold hover:bg-gray-50 transition-colors">Choose File...</button>
            ...
            <div className="border-b border-gray-100">
              <button onClick={() => toggleSection("content")} className="w-full flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3 font-semibold text-sm text-gray-700"><div className="w-6 h-6 rounded bg-emerald-50 text-emerald-600 flex items-center justify-center"><Type className="w-3.5 h-3.5" /></div>Text & Content</div>
                {activeSection === "content" ? <ChevronDown className="w-4 h-4 text-gray-400" /> : <ChevronRight className="w-4 h-4 text-gray-400" />}
              </button>
              {activeSection === "content" && (
                <div className="px-6 pb-6 space-y-4">
                  <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Main Headline</label><input value={headline} onChange={e => setHeadline(e.target.value)} className="w-full border rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-slate-900 outline-none" /></div>
                  <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Sub-Headline</label><textarea value={subheadline} onChange={e => setSubheadline(e.target.value)} rows={2} className="w-full border rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-slate-900 outline-none resize-none" /></div>
                  <div className="pt-4 border-t border-gray-100">
                    <div className="flex items-center gap-2 mb-3"><Wifi className="w-4 h-4 text-gray-400" /><span className="text-xs font-bold text-slate-700 uppercase tracking-wide">Wi-Fi Details (Optional)</span></div>
                    <div className="grid grid-cols-2 gap-3">
                      <input placeholder="Network Name" value={wifiSsid} onChange={e => setWifiSsid(e.target.value)} className="w-full border rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-slate-900 outline-none" />
                      <input placeholder="Password" value={wifiPass} onChange={e => setWifiPass(e.target.value)} className="w-full border rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-slate-900 outline-none" />
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="flex-1 bg-slate-100 border border-slate-200 rounded-3xl relative flex flex-col items-center justify-center p-10 overflow-hidden">
          ...
          <div className="no-print absolute top-5 right-5 bg-white px-4 py-1.5 rounded-full shadow-sm border border-slate-200/50 text-[10px] font-black uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
            <RefreshCw className="w-3 h-3 animate-spin" style={{ animationDuration: '4s' }} />Live Preview
          </div>

          <div className="shadow-2xl shadow-slate-400/30 transition-all duration-300 hover:scale-[1.005] select-none">
            <div className="bg-white" style={{ width: "148mm", height: "210mm" }}>
              <FlyerCard table={selectedTable} />
            </div>
          </div>
          ...
          {printMode === "bulk" && (
            <div className="no-print mt-4 text-[10px] font-bold text-slate-400 tracking-wider uppercase bg-slate-200/50 border border-slate-350/20 px-4 py-1.5 rounded-full">
              Bulk Print Active: loop printing {selectedTableIds.size} selected tables
            </div>
          )}
        </div>
      </div>
