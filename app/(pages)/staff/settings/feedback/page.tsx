...
import {
  ...
  ClipboardList, MessageSquare, Send, Zap,
} from "lucide-react";
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-wrap gap-2">
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${TYPE_COLORS[item.type]}`}>{TYPE_LABELS[item.type]}</span>
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${PRIORITY_COLORS[item.priority]}`}>{item.priority.charAt(0).toUpperCase() + item.priority.slice(1)} priority</span>
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${STATUS_COLORS[item.status]}`}>{STATUS_LABELS[item.status]}</span>
        </div>
      ...
      {expanded && <p className="mt-3 whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-sm text-slate-600 leading-relaxed border border-slate-100">{item.description}</p>}
    </div>
  );
}
...
export default function FeedbackPage() {
  ...
  useEffect(() => {
    (async () => {
      try { const res = await api<{ feedback: FeedbackEntry[] }>("/api/admin/feedback", { method: "GET" }); setHistory(res.feedback ?? []); } catch { } finally { setLoadingHistory(false); }
    })();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const newEntry = await api<FeedbackEntry>("/api/admin/feedback", { method: "POST", body: JSON.stringify({ type, priority, title: title.trim(), description: description.trim() }) });
  ...
  return (
    <SettingsPageLayout title="Feedback & Issues" description="Report bugs, request features, or share thoughts about Qrave." maxWidth="max-w-3xl">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-slate-600 uppercase tracking-wider">Type</label>
              <select value={type} onChange={(e) => setType(e.target.value as FeedbackType)} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm">
                <option value="bug">🐛 Bug Report</option><option value="feature_request">✨ Feature Request</option><option value="general">💬 General Feedback</option><option value="performance">⚡ Performance Issue</option><option value="ux">🎨 UX / Design</option>
              </select>
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-slate-600 uppercase tracking-wider">Priority</label>
              <select value={priority} onChange={(e) => setPriority(e.target.value as Priority)} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm">
                <option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option>
              </select>
            </div>
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-slate-600 uppercase tracking-wider">Title <span className="text-rose-500">*</span></label>
            <input type="text" value={title} maxLength={200} onChange={(e) => setTitle(e.target.value)} placeholder="Brief summary of the issue or request" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm" />
            <p className="mt-1 text-right text-xs text-slate-400">{title.length}/200</p>
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-slate-600 uppercase tracking-wider">Description <span className="text-rose-500">*</span></label>
            <textarea ref={descRef} value={description} maxLength={5000} onChange={(e) => setDescription(e.target.value)} rows={4}
              placeholder={type === "bug" ? "Steps to reproduce, expected vs actual behaviour…" : type === "feature_request" ? "Describe the feature you'd like…" : "Tell us what's on your mind…"}
              className="w-full resize-none overflow-hidden rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm"
            />
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="mb-5 text-base font-bold text-slate-900 flex items-center gap-2"><ClipboardList className="h-4 w-4 text-slate-500" />Your Previous Submissions</h2>
        {loadingHistory ? (
          ...
          <div className="space-y-3">{history.map((item) => <FeedbackCard key={item.id} item={item} />)}</div>
        )}
      </section>

      <div className="flex items-start gap-3 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3">
        <AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-blue-500" />
        <p className="text-xs text-blue-700 leading-relaxed">Feedback is reviewed by the Qrave team and tied to your restaurant account. Critical issues are triaged within 24 hours on business days.</p>
      </div>
    </SettingsPageLayout>
  );
}
